...
################ Light parameters #####################
wl     = 500
light_list  = [objects.Light(wl, max_order_PWs = 0, theta = 0.0, phi = 0.0)]
...
abs_mat = np.zeros((num_n,num_n))
for i in range(num_n):
    for j in range(num_n):
        abs_mat[j,i] = stacks_list[0][i][j].a_list[-1]
...
plt.title('Absorption of %(h)5.1f nm thick film @ wl = %(wl)5.1f'% \
    {'h' : stacks_list[0][0][0].heights_nm()[0],'wl' : wl})
